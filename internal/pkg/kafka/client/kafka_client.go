...
func NewKafkaClient(brokerConfig config.KafkaConfig) (sarama.Client, sarama.ClusterAdmin, sarama.ConsumerGroup, *sarama.Config) {
	...
	consumerGroup, err := sarama.NewConsumerGroupFromClient(brokerConfig.ConsumerGroupId, kafkaClient)
	if err != nil {
		easyzap.Fatal(err, "kafka client failed to new consumer group")
	}

	return kafkaClient, srClusterAdmin, consumerGroup, saramaConfig
}
